...
const LOCATION_PATTERNS = {
  // 주요 도시 및 역명 포함
  CITIES:
    /(서울|부산|대구|인천|광주|대전|울산|세종|고양|수원|안양|성남|용인|부천|안산|화성|파주|김포|천안|아산|천안아산|평택|김천|대전|익산|전주|광주|목포|부산|울산|포항|경주|안동|춘천|강릉|원주|청주|충주|제천|군산|순천|여수|창원|마산|진주|통영|사천|밀양|거제|양산)/i,

  // 구/동 및 주요 지역
  DISTRICTS:
    /(강남구|서초구|종로구|마포구|송파구|용산구|용산|영등포구|강서구|관악구|구로구|금천구|노원구|도봉구|동대문구|동작구|서대문구|성동구|성북구|양천구|은평구|중구|중랑구|일산동구|일산서구|분당구|덕양구|수지구|기흥구|처인구|팔달구|영통구|권선구|장안구)/i,

  // 도/광역시 (간단한 형태도 포함)
  PROVINCES:
    /(경기도|경기|강원도|강원|충청북도|충북|충청남도|충남|전라북도|전북|전라남도|전남|경상북도|경북|경상남도|경남|제주도|제주)/i,

  // 세부 지역 및 관광지
  DETAIL_AREAS:
    /(일산|분당|판교|동탄|송도|정자동|대화동|백석동|덕이동|청계동|명동|홍대|강남|역삼|선릉|삼성|잠실|가산|구로|영등포|여의도|마포|상암|성수|건대|왕십리|청량리|동대문|이태원|압구정|청담|논현|신사|한남|반포|서초|양재|사당|신림|노원|수유|창동|도봉|상계|중계|하계)/i,
};
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!textAnnotations || textAnnotations.length === 0) {
      console.log("Vision API - 텍스트를 찾을 수 없음, 기본값 반환");
      // 텍스트를 찾을 수 없어도 성공으로 처리하되 빈 결과 반환
      return NextResponse.json({
        success: true,
        extractedText: "",
        text: "",
...
function extractLocationInfo(text: string) {
  ...
  // 도/광역시 검색
  const provinceMatches = text.match(
    new RegExp(LOCATION_PATTERNS.PROVINCES, "g")
  );
  if (provinceMatches) {
  ...
  // 구/동 검색
  const districtMatches = text.match(
    new RegExp(LOCATION_PATTERNS.DISTRICTS, "g")
  );
  if (districtMatches) {
  ...
  // 상세 지역 검색
  const areaMatches = text.match(
    new RegExp(LOCATION_PATTERNS.DETAIL_AREAS, "g")
  );
  if (areaMatches) {
