...
export const gatheringService = {
  async list(page = 0, size = 10): Promise<PageResponse<GatheringPost>> {
    const res = await api.get('/api/groups', { params: { page, size } })
    const data = res.data
    return {
  ...
  async create(data: { title: string; content: string; maxCount?: number }): Promise<GatheringPost> {
  ...
  async update(id: number, data: { title: string; content: string; maxCount?: number }): Promise<GatheringPost> {
    const res = await api.put(`/api/groups/${id}`, {
      title: data.title,
      description: data.content,
      maxCount: data.maxCount ?? 10,
    })
